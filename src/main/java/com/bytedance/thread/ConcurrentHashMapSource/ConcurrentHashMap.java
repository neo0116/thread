...
// */
//
//package com.bytedance.thread.ConcurrentHashMapSource;
//
...
//            //ForwardingNode 节点，
//            //意味着有其他线程正在进行扩容，那么当前现在直接帮助它进行扩容，
...
//     */
//    /*
//    sizeCtl：当sizeCtl=0表示未初始化
//            当sizeCtl>0 表示初始化或者下一次扩容的大小
//            当sizeCtl = -1 表示正在初始化
//            当sizeCtl < -1 此时有  |sizeCtl| - |-1| 个线程在进行扩容
//    */
...
//        //引入CounterCell分片数组进行CAS，而不是单一的CAS，这样多线程下性能提高
...
//                //这里是生成一个唯一的扩容戳
//                //高 16 位代表扩容的标记、低 16 位代表并行扩容的线程数,具体看笔记
...
//                // 通过 rs<<RESIZE_STAMP_SHIFT 将 sc 设置 为一个负数，+2 表示有一个线程在执行扩容
...
//        // 判断此时是否仍然在执行扩容nextTab=null 的时候说明扩容已经结
...
//                // 那么 sc 无符号 右移比较高位和 rs 的值，那么应该是相等的。如果不相等，说明扩容结束了
//                //sc==rs+1 表示扩容结束
//                if ((sc >>> RESIZE_STAMP_SHIFT) != rs || sc == rs + 1 ||
...
//        //的话，默认一个 CPU（一个线程）处理 16 个桶，也就是长度为 16 的时候，扩容的时候只会有一 个线程来扩容
...
//                    ////每一个退出 transfer 的方法的线程，退出之前，会设置 sizeCtl = sizeCtl-1那么最后一个线程退出时：
...
//                            //将高位链表放在 i+n 位置
//
//                            //目的在于下次hash找node元素时tabAt能正确找到扩容后元素的位置，具体看笔记
//                            setTabAt(nextTab, i + n, hn);
//                            // 把旧 table 的 hash 桶中放置转发节 点，表明此 hash 桶已经被处理
//                            setTabAt(tab, i, fwd);
//                            advance = true;
//                        }
//                        else if (f instanceof TreeBin) {
...
//            //TODO 1
//            //说明 counterCells 已经被初始化过了，我们先跳过这个代码，先看初始化部分TODO 2
...
//                    collide = true;
//                //TODO 8
//                //进入这个步骤，说明 CounterCell 数组容量不够，线程竞争较大，所以先设置一个标识 表示为正在扩容
//                else if (cellsBusy == 0 &&
//                         U.compareAndSwapInt(this, CELLSBUSY, 0, 1)) {
//                    try {
